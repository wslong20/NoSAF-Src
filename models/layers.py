...
class SAGEBlock(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
        ...
        """
        aggr, normalize, root_weight, project, **kwargs are parameters of SAGEConv
        :param activation: str, "relu", "leaky_relu", "tanh", "none" or None
        :param norm: str, "batchnorm", "none" or None
        :param negative_slope: float, negative_slope of leaky_relu, if activation != "leaky_relu", this parameter is invalid
        """
        super().__init__()
        self.gnn = SAGEConv(in_features,
                            out_features,
                            bias=bias,
                            aggr=aggr,
                            normalize=normalize,
...
# class DNCoreNet(nn.Module):
#     def __init__(self, in_features, hid_features, out_features, numlayer, x_extractors, acc_extractors, node_weight_generator, acc_weight_generator, activation=None, norm=None, dropout=0.5):
#         super().__init__()
#         if numlayer <= 0:
#             raise ValueError("numlayer cannot be less than or equal to 0")
#         self.numlayer = numlayer
#         self.dropout = dropout
#
#         self.gnns = nn.ModuleList()
#         if numlayer == 1:
#             gnn = GNNBlock(in_features, out_features, bias=True, activation=activation, norm=norm)
#             self.gnns.append(gnn)
#         else:
#             self.gnns.append(GNNBlock(in_features, hid_features, bias=True, activation=activation, norm=norm))
#             for i in range(numlayer-2):
#                 gnn = GNNBlock(hid_features, hid_features, bias=True, activation=activation, norm=norm)
#                 self.gnns.append(gnn)
#             self.gnns.append(GNNBlock(hid_features, out_features, bias=True, activation=activation, norm=norm))
#
#         self.x_extractors = x_extractors
#         self.acc_extractors = acc_extractors
#         self.nwg = node_weight_generator
#         self.awg = acc_weight_generator
#
#         self.reset_parameters()
#
#     def reset_parameters(self):
#         self.x_extractors[0].reset_parameters()
#         for i in range(self.numlayer):
#             self.gnns[i].reset_parameters()
#             self.x_extractors[i+1].reset_parameters()
#             self.acc_extractors[i].reset_parameters()
#         self.nwg.reset_parameters()
#         self.awg.reset_parameters()
#
#     def forward(self, x, adj):
#         x_comp_info = self.x_extractors[0](x)
#         acc_comp_info = torch.zeros_like(x_comp_info)
#         weight_in = torch.cat([x_comp_info, acc_comp_info], dim=-1)
#         node_weight = F.sigmoid(self.nwg(weight_in))
#         acc = x * node_weight
#         x = x * node_weight
#         for i in range(self.numlayer):
#             x = self.gnns[i](x, adj)
#
#             x_comp_info = self.x_extractors[i+1](x)
#             acc_comp_info = self.acc_extractors[i](x)
#             weight_in = torch.cat([x_comp_info, acc_comp_info], dim=-1)
#             node_weight = F.sigmoid(self.nwg(weight_in))
#             acc_weight = F.sigmoid((self.awg(weight_in)))
#             x_nw = x * node_weight
#             x = acc * acc_weight + x_nw
#             acc = x_nw + acc
#
